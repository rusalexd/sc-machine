...
sc_event *event_operator_syncronizer_continue;
...
sc_result syncronize_scp_operator(const sc_event *event, sc_addr arg)
{
    ...
    if (SCP_RESULT_TRUE == ifCoin(s_default_ctx, &execute_result, &successfully_executed_scp_operator))
    {
        ...
        while (SCP_RESULT_TRUE == scp_iterator5_next(s_default_ctx, it, &operator_node, &arc3, &next_operator_node, &arc2, &nrel_then))
        {
            ...
            {
                // breakpoint check
                if (SCP_RESULT_TRUE == is_process_operator_breakpoint(&next_operator_node))
                {
                    scp_operand arc;
                    MAKE_DEFAULT_ARC_ASSIGN(arc);
                    if (SCP_RESULT_TRUE != genElStr3(s_default_ctx, &scp_paused_operator, &arc, &next_operator_node))
                    {
                        puts("======PAUSE");
                        return SCP_RESULT_ERROR;
                    }
...
sc_result countinue_paused_scp_operator(const sc_event *event, sc_addr arg)
{
    scp_operand arc, operand_node, arc_sub;
    MAKE_DEFAULT_OPERAND_FIXED(arc);
    MAKE_DEFAULT_NODE_ASSIGN(operand_node);
    arc.addr = arg;
    arc.element_type = scp_type_arc_pos_const_perm;
    MAKE_DEFAULT_ARC_ASSIGN(arc_sub);

    scp_iterator3 *it = scp_iterator3_new(s_default_ctx, &scp_unpaused_operator, &arc_sub, &operand_node);
    while (SCP_RESULT_TRUE == scp_iterator3_next(s_default_ctx, it, &scp_unpaused_operator, &arc_sub, &operand_node))
    {
        if (SCP_RESULT_TRUE == ifCoin(s_default_ctx, &arc_sub, &arc))
        {
            break;
        }
    }

    scp_operand prev_operand, arc2;
    MAKE_COMMON_ARC_ASSIGN(arc2);
    MAKE_DEFAULT_NODE_ASSIGN(prev_operand);

    operand_node.param_type = SCP_FIXED;
    scp_iterator3 *prev_it = scp_iterator3_new(s_default_ctx, &prev_operand, &arc2, &operand_node);
    while (SCP_RESULT_TRUE == scp_iterator3_next(s_default_ctx, prev_it, &prev_operand, &arc2, &operand_node))
    {
        prev_operand.param_type = SCP_FIXED;
        scp_operand arc_sub_1, arc_sub_2, arc_sub_3;
        MAKE_DEFAULT_ARC_ASSIGN(arc_sub_1);
        MAKE_DEFAULT_ARC_ASSIGN(arc_sub_1);
        MAKE_DEFAULT_ARC_ASSIGN(arc_sub_1);
        if (SCP_RESULT_TRUE == searchElStr3(s_default_ctx, &executed_scp_operator, &arc_sub_1, &prev_operand))
        {
            break;
        }
        if (SCP_RESULT_TRUE == searchElStr3(s_default_ctx, &successfully_executed_scp_operator, &arc_sub_2, &prev_operand))
        {
            break;
        }
        if (SCP_RESULT_TRUE == searchElStr3(s_default_ctx, &unsuccessfully_executed_scp_operator, &arc_sub_3, &prev_operand))
        {
            break;
        }
        prev_operand.param_type = SCP_ASSIGN;
    }

    scp_operand arc_del_1, arc_del_2;
    MAKE_DEFAULT_ARC_ASSIGN(arc_del_1);
    MAKE_DEFAULT_ARC_ASSIGN(arc_del_2);
    arc_del_1.erase = SCP_TRUE;
    arc_del_2.erase = SCP_TRUE;
    eraseElStr3(s_default_ctx, &scp_paused_operator, &arc_del_1, &operand_node);
    eraseElStr3(s_default_ctx, &scp_paused_operator, &arc_del_2, &operand_node);


    arc2.param_type = SCP_FIXED;
    return start_next_operator(s_default_ctx, &operand_node, &arc2);
}
...
scp_result scp_operator_syncronizer_init()
{
    ...
    if (event_operator_syncronizer_else == null_ptr)
        return SCP_RESULT_ERROR;
    event_operator_syncronizer_continue = sc_event_new(s_default_ctx, scp_unpaused_operator.addr, SC_EVENT_ADD_OUTPUT_ARC, 0, (fEventCallback)countinue_paused_scp_operator, 0);
    return SCP_RESULT_TRUE;
}
