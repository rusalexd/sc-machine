...
sc_result create_scp_process(const sc_event *event, sc_addr arg)
{
    ...
    MAKE_DEFAULT_ARC_ASSIGN(arc4);
    if(SCP_RESULT_TRUE == searchElStr3(s_default_ctx, &scp_procedure_node, &arc4, &scp_debug_execution_program)){
        MAKE_DEFAULT_ARC_ASSIGN(arc4);
        genElStr3(s_default_ctx, &scp_process, &arc4, &scp_debug_executing_process);
    }
